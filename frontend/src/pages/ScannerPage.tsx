...
export function ScannerPage() {
  ...
  const enabledRuleIds = useMemo(() => (rules.data ?? []).filter((item) => item.enabled).map((item) => item.id), [rules.data])
  const allRulesSelected = enabledRuleIds.length > 0 && enabledRuleIds.every((id) => selectedRules.includes(id))
  ...
  return <div className="page-stack">
    ...
    <section className="scanner-layout">
      <aside className="filter-panel">
        ...
        <fieldset><legend>规则</legend><div className="fieldset-actions"><span>已选 {selectedRules.length}/{enabledRuleIds.length}</span><button type="button" onClick={() => setSelectedRules(allRulesSelected ? [] : enabledRuleIds)}>{allRulesSelected ? '清空' : '全选'}</button></div><div className="check-list">{rules.data!.filter((item) => item.enabled).map((item) => <label key={item.id}><input type="checkbox" checked={selectedRules.includes(item.id)} onChange={() => toggle(item.id, selectedRules, setSelectedRules)} /><span><strong>{item.name}</strong><small>{item.category}</small></span></label>)}</div></fieldset>
      </aside>
