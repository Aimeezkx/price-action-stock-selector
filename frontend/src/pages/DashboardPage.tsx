...
export function DashboardPage() {
  ...
  return <div className="page-stack">
    ...
    <section className="connection-strip"><span className={data.digest.configured && data.digest.enabled ? 'connection-icon online' : 'connection-icon'}><Mail size={18} /></span><div><strong>候选邮件 · {data.digest.daily_time} {data.digest.timezone}</strong><p>收件人 {data.digest.recipient} · {data.digest.configured ? (data.digest.enabled ? `已启用，下次 ${formatNextRun(data.digest.next_run_at)}` : 'SMTP 已配置，调度未启用') : '等待安全配置 Gmail SMTP 应用密码'}</p></div></section>
  </div>
}
