...
export function BacktestPage() {
  ...
  return <div className="page-stack">
    ...
    <section className="backtest-config panel"><label><span>规则</span><select value={ruleId} onChange={(event) => setRuleId(event.target.value)}>{rules.data!.filter((item) => item.direction === 'long').map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select></label><label><span>最长持仓天数</span><input type="number" min="1" max="120" value={holdingDays} onChange={(event) => setHoldingDays(Number(event.target.value))} /></label><label><span>入场有效天数</span><input type="number" min="1" max="10" value={entryExpiryDays} onChange={(event) => setEntryExpiryDays(Number(event.target.value))} /></label><label><span>目标 R</span><input type="number" min="0.25" max="10" step="0.25" value={targetR} onChange={(event) => setTargetR(Number(event.target.value))} /></label><button className="button primary" onClick={() => run.mutate()} disabled={run.isPending}><Play size={17} />{run.isPending ? '计算中…' : '运行回测'}</button></section>
